...
{
    private CardData m_persistentData; // 영구 저장될 원본 데이터 (아이템 슬롯 적용)
    private CardData m_battleData;     // 이번 전투에만 사용될 임시 데이터 (디버프 벌레 적용)
    ...
    /// <summary>
    /// 공격 이후 호출되어 아이템 효과(내구도, Boom 벌레)를 처리합니다.
    /// </summary>
    ...
    private void CreatePortrait(bool isUI)
    {
        ...
        // 초상화는 전투용 데이터(디버프 시 초상화가 변경될 수 있으므로)를 기준으로 생성
