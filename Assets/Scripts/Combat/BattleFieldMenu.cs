...
public class BattleFieldMenu : MonoBehaviour
{
    ...
    public void InitTeamStatus(List<BattleCard> teamCards)
    {
        ...
        // 각 팀원에 맞게 UI 생성 및 위치 설정
        for (int i = 0; i < memberCount; i++)
        {
            if (teamCards[i] != null)
            {
                // m_teamStatusParent의 왼쪽부터 시작해서 오브젝트로부터 간격만큼 배치
    ...
    /// <summary>
    /// 특정 팀원의 상태를 업데이트합니다. 현재는 HP바만 업데이트합니다.
    /// </summary>
    /// <param name="teamCard">업데이트할 팀 카드</param>
    /// <param name="index">리스트에서의 인덱스</param>
    ...
    /// <summary>
    /// 팀원의 초상화를 생성합니다. 원본 카드의 초상화를 참조하여 사용합니다.
    /// </summary>
